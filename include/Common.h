...
#define MAX_APP_THREAD                                                         \
  65 // one additional thread for data statistics(main thread)  [CONFIG] 65
...
constexpr uint32_t leafSpanSize =
    16;                          // 16  NOTE: this affects the bandwidth/IOPS
...
constexpr uint64_t kKeyMax =
    60000000; // only for int workloads [CONFIG] 60000000
#endif
...
constexpr uint32_t entryGroupNum =
    leafSpanSize / neighborSize + (leafSpanSize % neighborSize);
...
constexpr int decodedLeafSize =
    (leafMetadataSize + leafEntrySize * neighborSize) * entryGroupNum;
#else
...
constexpr uint32_t transLeafSize =
    (decodedLeafSize <= cachelineSize)
        ? decodedLeafSize
        : (cachelineSize + ADD_CACHELINE_VERSION_SIZE(
                               decodedLeafSize - cachelineSize, versionSize));
constexpr uint32_t allocationLockSize = 16UL; // 16 round up lock_addr
constexpr uint32_t allocationLeafSize =
    transLeafSize + allocationLockSize; // remain space for the lock
...
constexpr uint32_t bufferEntrySize =
    ADD_CACHELINE_VERSION_SIZE(leafMetadataSize + leafEntrySize, versionSize);
constexpr uint32_t bufferMetadataSize =
    ADD_CACHELINE_VERSION_SIZE(leafMetadataSize, versionSize);
...
// Synonym leaf
constexpr uint64_t leafNumMax = 1200000000 / leafSpanSize; // [CONFIG]
constexpr uint64_t synRegionOffset =
    kLeafRegionStartOffset +
    ROUND_UP(leafNumMax * ROUND_UP(define::allocationLeafSize, 3),
             CACHELINE_ALIGN_BIT); // B
} // namespace define
