...
using leaf_table_t =
    rolex::LeafTable<__uint128_t, Value, LeafNode, leaf_alloc_t>;
using rolex_t = rolex::Rolex<__uint128_t, Value, LeafNode, leaf_alloc_t,
                             remote_memory_t, define::epsilon>;
#else
using model_alloc_t = rolex::ModelAllocator<uint64_t>;
using remote_memory_t = rolex::RemoteMemory<leaf_alloc_t, model_alloc_t>;
using leaf_table_t = rolex::LeafTable<uint64_t, Value, LeafNode, leaf_alloc_t>;
using rolex_t = rolex::Rolex<uint64_t, Value, LeafNode, leaf_alloc_t,
                             remote_memory_t, define::epsilon>;
#endif
...
inline RolexCache::RolexCache(DSM *dsm, const std::vector<Key> &load_keys)
    : dsm(dsm) {
  // processing data
#ifdef USE_INT_128
  for (const auto &k : load_keys)
    int_keys.emplace_back(key2int128(k));
#else
  for (const auto &k : load_keys)
    int_keys.emplace_back(key2int(k));
#endif
  ...
  rolex::RM_config conf(ctrl, define::modelRegionSize,
                        define::fakeLeafRegionSize, define::fakeRegLeafRegion);
  remote_memory_t *RM = new remote_memory_t(conf);
  rolex_model = new rolex_t(RM, int_keys);
}

inline std::pair<int, int>
RolexCache::search_from_cache(const Key &k) { // [l, r]
#ifdef USE_INT_128
...
inline std::tuple<int, int, int> RolexCache::search_from_cache_for_insert(
    const Key &k) { // [l, r, insert_leaf_idx]
#ifdef USE_INT_128
  auto key = key2int128(k);
#else
  auto key = key2int(k);
#endif
  auto [l, r, leaf_idx_offset, capacity_offset] =
      rolex_model->get_leaf_range(key);
  int global_key_idx = std::lower_bound(int_keys.begin(), int_keys.end(), key) -
                       int_keys.begin();
  ...
  return std::make_tuple(leaf_idx_offset + l, leaf_idx_offset + r,
                         leaf_idx_offset + insert_idx);
}

inline std::pair<int, int>
RolexCache::search_range_from_cache(const Key &from, const Key &to) { // [l, r]
#ifdef USE_INT_128
...
#endif
  auto [l, _1, leaf_idx_offset_from, _2] =
      rolex_model->get_leaf_range(from_key);
