...
thread_local std::map<GlobalAddress, GlobalAddress>
    RolexIndex::coro_syn_leaf_addrs[MAX_CORO_NUM + 1];

RolexIndex::RolexIndex(DSM *dsm, std::vector<Key> &load_keys, uint16_t rolex_id)
    : dsm(dsm), rolex_id(rolex_id) {
...
inline GlobalAddress RolexIndex::get_leaf_address(uint64_t leaf_idx) {
  uint64_t offset =
      define::kLeafRegionStartOffset +
      (leaf_idx / MEMORY_NODE_NUM) * ROUND_UP(define::allocationLeafSize, 3);
...
void RolexIndex::lock_node(const GlobalAddress &node_addr,
                           uint64_t *lock_buffer, CoroPull *sink) {
  auto lock_offset = get_lock_info();

  // lock function
  auto acquire_lock = [=](const GlobalAddress &node_addr) {
    return dsm->cas_mask_sync(node_addr + lock_offset, 0ULL, ~0ULL, lock_buffer,
                              1ULL << 63, ~0ULL, sink);
  };
  int cnt = 0;
re_acquire:
...
void RolexIndex::unlock_node(const GlobalAddress &node_addr,
                             uint64_t *lock_buffer, CoroPull *sink,
                             bool async) {
  auto lock_offset = get_lock_info();
  ...
  auto unlock = [=](const GlobalAddress &node_addr) {
    if (async) {
      dsm->write((char *)lock_buffer, node_addr + lock_offset, sizeof(uint64_t),
                 false, sink);
    } else {
      dsm->write_sync((char *)lock_buffer, node_addr + lock_offset,
                      sizeof(uint64_t), sink);
    }
  };
  unlock(node_addr);
  return;
...
void RolexIndex::insert(const Key &k, Value v, CoroPull *sink) {
...

  {
    ...
    for (int i = l; i <= r; ++i)
      leaf_addrs.emplace_back(
          get_leaf_address(i)); // without reading synonym leaves
    read_leaf_cnt += leaf_addrs.size();
#ifdef HOPSCOTCH_LEAF_NODE
    int hash_idx = get_hashed_leaf_entry_index(k);
    hopscotch_fetch_nodes(
        leaf_addrs, hash_idx, _, sink,
        std::vector<int>(leaf_addrs.size(), define::neighborSize));
#else
    ...
    int read_entry_num = define::leafSpanSize,
        read_synonym_entry_num = define::leafSpanSize;
    ...
    auto get_read_entry_num = [=](bool is_synonym) {
      ...
      int l_idx = hash_idx;
      int entry_num =
          if_lock->get_read_entry_num_from_bitmap(l_idx, is_synonym);
      ...
      if (entry_num < (int)define::neighborSize &&
          ...
          (l_idx / define::neighborSize ==
           (r_idx - 1) / define::neighborSize)) {
        r_idx = (r_idx - 1 + define::neighborSize) / define::neighborSize *
                    define::neighborSize +
                1;
    ...
    int r_synonym_idx =
        (hash_idx + read_synonym_entry_num) % define::leafSpanSize;
#endif
    ...
    if (syn_leaf_addrs.find(insert_leaf_addr) == syn_leaf_addrs.end()) {
      read_leaf_cnt++;
#if (defined HOPSCOTCH_LEAF_NODE && defined VACANCY_AWARE_LOCK)
      hopscotch_fetch_node(insert_leaf_addr, hash_idx, leaf, sink,
                           read_entry_num, false);
#else
      ...
      if (leaf->metadata.synonym_ptr != GlobalAddress::Null()) {
        ...
        read_leaf_cnt++;
#if (defined HOPSCOTCH_LEAF_NODE && defined VACANCY_AWARE_LOCK)
        hopscotch_fetch_node(syn_leaf_addrs[insert_leaf_addr], hash_idx,
                             syn_leaf, sink, read_synonym_entry_num);
#else
...
#if (defined HOPSCOTCH_LEAF_NODE && defined VACANCY_AWARE_LOCK)
      hopscotch_fetch_nodes(
          std::vector<GlobalAddress>{insert_leaf_addr,
                                     syn_leaf_addrs[insert_leaf_addr]},
          hash_idx, two_leaves, sink,
          std::vector<int>{read_entry_num, read_synonym_entry_num});
#else
      fetch_nodes(std::vector<GlobalAddress>{insert_leaf_addr,
                                             syn_leaf_addrs[insert_leaf_addr]},
                  two_leaves, sink);
#endif
    ...
    for (int i = hash_idx; i != r_idx; i = (i + 1) % define::leafSpanSize)
      if (records[i].key == k) { // existing key
        unlock_node(insert_leaf_addr, lock_buffer, sink);
        goto insert_finish;
      }
#else
    for (const auto &e : leaf->records)
      if (e.key == k) { // existing key
        unlock_node(insert_leaf_addr, lock_buffer, sink);
        goto insert_finish;
      }
#endif
    ...
    if (!hopscotch_insert_and_unlock(
            (LeafNode *)leaf_copy_buffer, k, v, insert_leaf_addr, lock_buffer,
            sink, read_entry_num)) { // return false(and remain locked) if need
                                     // insert into synonym leaf
      ...
      if (!syn_leaf) { // allocate a new synonym leaf
        ...
        if (read_entry_num < define::leafSpanSize) {
          LeafNode *tmp_leaf;
          hopscotch_fetch_node(insert_leaf_addr, r_idx, tmp_leaf, sink,
                               define::leafSpanSize - read_entry_num, false,
                               false);
...
#endif
        hopscotch_split_and_unlock(leaf, k, v, insert_leaf_addr, lock_buffer,
                                   sink);
      } else { // insert into old synonym leaf
        if (!hopscotch_insert_and_unlock(syn_leaf, k, v, syn_leaf_addr,
                                         lock_buffer, sink,
                                         read_synonym_entry_num, false)) {
          printf("synonmy leaf is hop-full!!\n"); // ASSERT: synonmy leaf is
                                                  // hop-full!!
          // assert(false);
        }
        unlock_node(insert_leaf_addr, lock_buffer, sink);
    ...
    if (i == (int)define::leafSpanSize) { // insert k into the synonym leaf
      write_syn_leaf = true;
      auto syn_addr = insert_into_syn_leaf_locally(
          k, v, syn_leaf, insert_leaf_addr.nodeID, sink);
      ...
      while (j < (int)define::leafSpanSize &&
             records[j].key != define::kkeyNull)
        j++;
      if (j ==
          (int)
              define::leafSpanSize) { // overflow the last k to the synonym leaf
        write_syn_leaf = true;
        const auto &last_e = records[j - 1];
        auto syn_addr = insert_into_syn_leaf_locally(
            last_e.key, last_e.value, syn_leaf, insert_leaf_addr.nodeID, sink);
        if (syn_addr == GlobalAddress::Max()) { // existing key
          unlock_node(insert_leaf_addr, lock_buffer, sink);
          goto insert_finish;
        }
    ...
    if (write_syn_leaf)
      leaf_addrs.emplace_back(syn_leaf_addrs[insert_leaf_addr]),
          leaves.emplace_back(syn_leaf);
    write_nodes_and_unlock(leaf_addrs, leaves, insert_leaf_addr, lock_buffer,
                           sink);
    // unlock_node(insert_leaf_addr, lock_buffer, sink);
#endif
  }
...
GlobalAddress RolexIndex::insert_into_syn_leaf_locally(const Key &k, Value v,
                                                       LeafNode *&syn_leaf,
                                                       int nodeID,
                                                       CoroPull *sink) {
...
void RolexIndex::fetch_nodes(const std::vector<GlobalAddress> &leaf_addrs,
                             std::vector<LeafNode *> &leaves, CoroPull *sink,
                             bool update_local_slt) {
...
re_fetch:
  ...
  for (int i = 0; i < leaf_addrs.size(); ++i) {
#ifdef METADATA_REPLICATION
    if (!(LeafVersionManager::decode_node_versions(raw_buffers[i],
                                                   intermediate_buffers[i]))) {
      read_leaf_retry[dsm->getMyThreadID()]++;
      goto re_fetch;
    }
    MetadataManager::decode_node_metadata(intermediate_buffers[i],
                                          (char *)leaves[i]);
#else
  ...
  if (update_local_slt)
    for (int i = 0; i < (int)leaf_addrs.size(); ++i) {
      if (leaves[i]->metadata.synonym_ptr != GlobalAddress::Null()) {
        coro_syn_leaf_addrs[sink ? sink->get() : 0][leaf_addrs[i]] =
            leaves[i]->metadata.synonym_ptr;
      }
    }
  return;
}

void RolexIndex::write_nodes_and_unlock(
    const std::vector<GlobalAddress> &leaf_addrs,
    const std::vector<LeafNode *> &leaves,
    const GlobalAddress &locked_leaf_addr, uint64_t *lock_buffer,
    CoroPull *sink) {
  ...
  for (int i = 0; i < (int)leaf_addrs.size(); ++i) {
#ifdef METADATA_REPLICATION
    MetadataManager::encode_node_metadata((char *)leaves[i],
                                          intermediate_leaf_buffers[i]);
    LeafVersionManager::encode_node_versions(intermediate_leaf_buffers[i],
                                             encoded_leaf_buffers[i]);
#else
...
void RolexIndex::fetch_node(const GlobalAddress &leaf_addr, LeafNode *&leaf,
                            CoroPull *sink, bool update_local_slt) {
  try_read_leaf[dsm->getMyThreadID()]++;
...
re_read:
  ...
  // consistency check
#ifdef METADATA_REPLICATION
  if (!(LeafVersionManager::decode_node_versions(raw_buffer,
                                                 intermediate_buffer))) {
    read_leaf_retry[dsm->getMyThreadID()]++;
    goto re_read;
  }
  ...
  if (update_local_slt)
    if (leaf->metadata.synonym_ptr != GlobalAddress::Null()) {
      coro_syn_leaf_addrs[sink ? sink->get() : 0][leaf_addr] =
          leaf->metadata.synonym_ptr;
    }
  return;
}

void RolexIndex::write_node_and_unlock(const GlobalAddress &leaf_addr,
                                       LeafNode *leaf,
                                       const GlobalAddress &locked_leaf_addr,
                                       uint64_t *lock_buffer, CoroPull *sink) {
  auto encoded_leaf_buffer = (dsm->get_rbuf(sink)).get_leaf_buffer();
#ifdef METADATA_REPLICATION
  ...
  LeafVersionManager::encode_node_versions(intermediate_leaf_buffer,
                                           encoded_leaf_buffer);
#else
...
#ifdef HOPSCOTCH_LEAF_NODE
    auto p = std::make_pair(hash_idx, (hash_idx + define::neighborSize) %
                                          define::leafSpanSize);
#else
    auto p = std::make_pair(0, define::leafSpanSize);
#endif
    if (speculative_read(leaf_addr, p, k, old_v, leaf, kv_idx, read_leaf_cnt,
                         sink)) {
      UNUSED(old_v);
      goto update_entry;
    }
    if (speculative_read(leaf_addr,
                         std::make_pair(p.first + define::leafSpanSize,
                                        p.second + define::leafSpanSize),
                         k, v, leaf, kv_idx, read_leaf_cnt, sink)) {
      UNUSED(old_v);
      goto update_entry;
    }
    leaf_addr = old_addr;
...
#ifdef SPECULATIVE_READ
    idx_cache->add_to_cache(leaf_addr,
                            (leaf_addr == lock_leaf_addr)
                                ? kv_idx
                                : (define::leafSpanSize + kv_idx),
                            k);
  update_entry:
    ...
    dsm->write_sync(block_buffer, block_addr, define::dataBlockLen, sink);
    // change value into the DataPointer value pointing to the DataBlock
    target_e.update(target_e.key,
                    (uint64_t)DataPointer(define::dataBlockLen, block_addr));
#endif

#ifdef HOPSCOTCH_LEAF_NODE
    entry_write_and_unlock(leaf, kv_idx, leaf_addr, lock_leaf_addr, lock_buffer,
                           sink);
#else
...
bool RolexIndex::search(const Key &k, Value &v, CoroPull *sink) {
  ...
  lock_res =
      local_lock_table->acquire_local_read_lock(k, &busy_waiting_queue, sink);
...
std::tuple<bool, GlobalAddress, GlobalAddress, int>
RolexIndex::_search(const Key &k, Value &v, CoroPull *sink) {
  int read_leaf_cnt = 0;
#ifdef SYN_CACHE
  auto &syn_leaf_addrs = coro_syn_leaf_addrs[sink ? sink->get() : 0];
#else
  ...
  for (int i = l; i <= r; ++i) {
    ...
    auto p = std::make_pair(0, define::leafSpanSize);
#endif
    if (speculative_read(leaf_addr, p, k, v, leaf, kv_idx, read_leaf_cnt,
                         sink)) {
      return std::make_tuple(true, leaf_addr, get_leaf_address(i),
                             read_leaf_cnt);
    }
    if (speculative_read(leaf_addr,
                         std::make_pair(p.first + define::leafSpanSize,
...
re_read:
...
#ifdef HOPSCOTCH_LEAF_NODE
  hopscotch_fetch_nodes(
      leaf_addrs, hash_idx, leaves, sink,
      std::vector<int>(leaf_addrs.size(), define::neighborSize), false);
#else
...
#ifdef HOPSCOTCH_LEAF_NODE
    hopscotch_fetch_nodes(
        append_leaf_addrs, hash_idx, append_leaves, sink,
        std::vector<int>(append_leaf_addrs.size(), define::neighborSize));
#else
    fetch_nodes(append_leaf_addrs, append_leaves, sink);
#endif
    leaf_addrs.insert(leaf_addrs.end(), append_leaf_addrs.begin(),
                      append_leaf_addrs.end());
    leaves.insert(leaves.end(), append_leaves.begin(), append_leaves.end());
    locked_leaf_addrs.insert(locked_leaf_addrs.end(),
                             append_locked_leaf_addrs.begin(),
                             append_locked_leaf_addrs.end());
  }
  // 3. Search the fetched leaves
  assert(leaf_addrs.size() == leaves.size() &&
         leaves.size() == locked_leaf_addrs.size());
  for (int i = 0; i < (int)leaves.size(); ++i) {
    ...
    for (int j = 0; j < (int)define::neighborSize; ++j) {
      ...
      if (e.key != define::kkeyNull &&
          (int)get_hashed_leaf_entry_index(e.key) == hash_idx) {
        hop_bitmap |= 1ULL << (define::neighborSize - j - 1);
        if (e.key == k) { // optimization: if the target key is found,
                          // consistency check can be stopped
          v = e.value;
#ifdef SPECULATIVE_READ
          idx_cache->add_to_cache(leaf_addrs[i],
                                  (leaf_addrs[i] == locked_leaf_addrs[i])
                                      ? kv_idx
                                      : (define::leafSpanSize + kv_idx),
                                  k);
#endif
...
*/
void RolexIndex::range_query(
    const Key &from, const Key &to,
    std::map<Key, Value> &ret) { // [from, to)  TODO: HOPSCOTCH_LEAF_NODE
  assert(dsm->is_register());
  ...
  std::vector<LeafNode *> leaves;
  for (int i = l; i <= r; ++i)
    leaf_addrs.emplace_back(get_leaf_address(i)); // leaves
  ...
  for (auto &[_, v] : ret) {
    auto data_block =
        (DataBlock *)(range_buffer + kv_cnt * define::dataBlockLen);
    v = data_block->value;
    kv_cnt++;
  }
#endif
...
#ifdef HOPSCOTCH_LEAF_NODE
bool RolexIndex::hopscotch_insert_and_unlock(
    LeafNode *leaf, const Key &k, Value v, const GlobalAddress &node_addr,
    uint64_t *lock_buffer, CoroPull *sink, int entry_num, bool is_locked_leaf) {
  auto &records = leaf->records;
...
next_hop:
  ...
  if (j < hash_idx + (int)define::neighborSize) {
    ...
    segment_write_and_unlock(leaf, hash_idx, empty_idx % define::leafSpanSize,
                             hopped_idxes, node_addr, lock_buffer, sink,
                             is_locked_leaf);
    return true;
  }
  for (int offset = define::neighborSize - 1; offset > 0; --offset) {
    ...
    // corner case
    if (h - h_hash_idx < 0)
      h_hash_idx -= define::leafSpanSize;
    else if (h - h_hash_idx >= (int)define::neighborSize)
      h_hash_idx += define::leafSpanSize;
    assert(h_hash_idx <= h);
    // hop h => j is ok
...
void RolexIndex::hopscotch_split_and_unlock(LeafNode *leaf, const Key &k,
                                            Value v,
  ...
  // synonym node
  auto synonym_addr = dsm->alloc(define::allocationLeafSize,
                                 CACHELINE_ALIGN_BIT, node_addr.nodeID);
  ...
  for (int i = 0; i < (int)define::leafSpanSize; ++i) {
    ...
    if (old_e.key != define::kkeyNull) {
      ...
      if (old_e.key >= split_key) {
        ...
        // update hop_bit
        auto offset =
            (i >= hash_idx ? i - hash_idx
                           : i + (int)define::leafSpanSize - hash_idx);
        synonym_leaf->records[hash_idx].set_hop_bit(offset);
        records[hash_idx].unset_hop_bit(offset);
      }
    }
  }
  load_factor_sum[dsm->getMyThreadID()] +=
      (double)non_empty_entry_cnt / define::leafSpanSize;
  // newly insert kv
  if (k < split_key)
    hopscotch_insert_locally(records, k, v);
  else
    hopscotch_insert_locally(synonym_leaf->records, k, v);
  // change metadata
  ...
  MetadataManager::encode_node_metadata(synonym_buffer,
                                        intermediate_synonym_buffer);
  LeafVersionManager::encode_node_versions(intermediate_synonym_buffer,
                                           encoded_synonym_buffer);
#else
  VerMng::encode_node_versions(synonym_buffer, encoded_synonym_buffer);
#endif
  dsm->write_sync(encoded_synonym_buffer, synonym_addr, define::transLeafSize,
                  sink);
...
#endif
  auto lock_offset = get_lock_info();
  memcpy(encoded_leaf_buffer + lock_offset, lock_buffer,
         sizeof(uint64_t)); // unlock
  dsm->write_sync(encoded_leaf_buffer, node_addr,
                  define::transLeafSize + define::allocationLockSize, sink);
}
...
Key RolexIndex::hopscotch_get_split_key(LeafEntry *records, const Key &k) {
  ...
  int hash_idx = get_hashed_leaf_entry_index(k);
  for (int empty_idx = hash_idx;
       empty_idx < hash_idx + (int)define::leafSpanSize; ++empty_idx) {
    if (get_entry(empty_idx).key == define::kkeyNull)
      break;
...
void RolexIndex::hopscotch_insert_locally(LeafEntry *records, const Key &k,
                                          Value v) {
  auto get_entry = [=, &records](int logical_idx) -> LeafEntry & {
    return records[(logical_idx + define::leafSpanSize) % define::leafSpanSize];
  };
...
void RolexIndex::hopscotch_fetch_node(const GlobalAddress &leaf_addr,
                                      int hash_idx, LeafNode *&leaf,
                                      CoroPull *sink, int entry_num,
                                      bool update_local_slt,
                                      bool need_metadata) {
  std::vector<LeafNode *> leaves;
  hopscotch_fetch_nodes(std::vector<GlobalAddress>{leaf_addr}, hash_idx, leaves,
                        sink, std::vector<int>{entry_num}, update_local_slt,
                        need_metadata);
  leaf = leaves.front();
  return;
}

void RolexIndex::hopscotch_fetch_nodes(
    const std::vector<GlobalAddress> &leaf_addrs, int hash_idx,
    std::vector<LeafNode *> &leaves, CoroPull *sink,
    std::vector<int> entry_nums, bool update_local_slt, bool need_metadata) {
  try_read_leaf[dsm->getMyThreadID()]++;
  std::vector<char *> raw_buffers;
  std::vector<RdmaOpRegion> rs;
  ...
  for (int i = 0; i < leaf_addrs.size(); ++i) {
    ...
    auto segment_size_r =
        std::min(entry_nums[i], (int)define::leafSpanSize - hash_idx);
    auto segment_size_l =
        entry_nums[i] <= (int)define::leafSpanSize - hash_idx
            ? 0
            : entry_nums[i] - ((int)define::leafSpanSize - hash_idx);
    segment_sizes_r.emplace_back(segment_size_r),
        segment_sizes_l.emplace_back(segment_size_l);
#ifdef METADATA_REPLICATION
    ...
    auto [raw_offset_r, raw_len_r, first_offset_r] =
        LeafVersionManager::get_offset_info(hash_idx, segment_size_r);
    auto [raw_offset_l, raw_len_l, first_offset_l] =
        LeafVersionManager::get_offset_info(0, segment_size_l);
#else
    auto [raw_offset_r, raw_len_r, first_offset_r] =
        VerMng::get_offset_info(hash_idx, segment_size_r);
    auto [raw_offset_l, raw_len_l, first_offset_l] =
        VerMng::get_offset_info(0, segment_size_l);
#endif
    assert(segment_size_l > 0 || !raw_len_l);
    raw_offsets_r.emplace_back(raw_offset_r),
        raw_offsets_l.emplace_back(raw_offset_l);
    raw_lens_r.emplace_back(raw_len_r), raw_lens_l.emplace_back(raw_len_l);
    first_offsets_r.emplace_back(first_offset_r),
        first_offsets_l.emplace_back(first_offset_l);
  }
...
re_fetch:
  ...
  for (int i = 0; i < leaf_addrs.size(); ++i) {
    if (raw_lens_l[i]) { // read two hop segments (corner case)
      ...
      auto [first_metadata_offset_l, new_len_l] =
          MetadataManager::get_offset_info(0, segment_sizes_l[i]);
      auto [first_metadata_offset_r, new_len_r] =
          MetadataManager::get_offset_info(hash_idx, segment_sizes_r[i]);
      if (!LeafVersionManager::decode_segment_versions(
              raw_segment_buffer_l, intermediate_buffers_l[i],
              first_offsets_l[i], segment_sizes_l[i], first_metadata_offset_l,
              new_len_l, segment_node_versions_l) ||
          !LeafVersionManager::decode_segment_versions(
              raw_segment_buffer_r, intermediate_buffers_r[i],
              first_offsets_r[i], segment_sizes_r[i], first_metadata_offset_r,
              new_len_r, segment_node_versions_r) ||
          segment_node_versions_r !=
              segment_node_versions_l) { // consistency check
        read_leaf_retry[dsm->getMyThreadID()]++;
        goto re_fetch;
      }
      bool has_metadata_l = MetadataManager::decode_segment_metadata(
          intermediate_buffers_l[i], (char *)&(leaves[i]->records[0]),
          first_metadata_offset_l, segment_sizes_l[i], metadata_l);
      bool has_metadata_r = MetadataManager::decode_segment_metadata(
          intermediate_buffers_r[i], (char *)&(leaves[i]->records[hash_idx]),
          first_metadata_offset_r, segment_sizes_r[i], metadata_r);
      assert(!need_metadata || has_metadata_l || has_metadata_l);
      if (has_metadata_l && has_metadata_r)
        assert(metadata_l == metadata_r);
      ...
      auto has_metadata = MetadataManager::decode_segment_metadata(
          intermediate_buffers_r[i], (char *)&(leaves[i]->records[hash_idx]),
          first_metadata_offset_r, segment_sizes_r[i], leaves[i]->metadata);
...
re_fetch:
  ...
  for (int i = 0; i < leaf_addrs.size(); ++i) {
    ...
    r1.size =
        raw_offsets_l[i] + raw_lens_l[i]; // header + segment_l (corner case)
    r1.is_on_chip = false;
    rs.emplace_back(r1);
  ...
  for (int i = 0; i < leaf_addrs.size(); ++i) {
    ...
    auto raw_segment_buffer_r = raw_buffer + raw_offsets_r[i];
    auto raw_segment_buffer_l = raw_buffer + raw_offsets_l[i];
    uint8_t metadata_node_version = 0, segment_node_versions_r = 0,
            segment_node_versions_l = 0;
    if (!VerMng::decode_header_versions(raw_buffer, (char *)leaves[i],
                                        metadata_node_version) ||
        (segment_sizes_l[i] > 0 &&
         !VerMng::decode_segment_versions(
             raw_segment_buffer_l, (char *)&(leaves[i]->records[0]),
             first_offsets_l[i], segment_sizes_l[i],
             segment_node_versions_l)) ||
        !VerMng::decode_segment_versions(
            raw_segment_buffer_r, (char *)&(leaves[i]->records[hash_idx]),
            first_offsets_r[i], segment_sizes_r[i], segment_node_versions_r) ||
        metadata_node_version != segment_node_versions_r ||
        (segment_sizes_l[i] > 0 &&
         metadata_node_version != segment_node_versions_l)) {
      read_leaf_retry[dsm->getMyThreadID()]++;
      goto re_fetch;
    }
  }
...
void RolexIndex::segment_write_and_unlock(LeafNode *leaf, int l_idx, int r_idx,
                                          const std::vector<int> &hopped_idxes,
                                          const GlobalAddress &node_addr,
                                          uint64_t *lock_buffer, CoroPull *sink,
                                          bool need_unlock) {
  auto &records = leaf->records;
  ...
  auto get_empty_idxes = [=, &empty_idxes](int l, int r) {
    for (int i = l; i < r; ++i)
      if (leaf->records[i].key == define::kkeyNull) {
        empty_idxes.emplace_back(i);
      }
  };
  if (l_idx <= r_idx)
    get_empty_idxes(l_idx, r_idx + 1);
  else {
  ...
  if (l_idx <= r_idx) { // update with one WRITE + unlock
...
#ifdef METADATA_REPLICATION
    // segment [l_idx, r_idx]
    auto intermediate_segment_buffer =
        (dsm->get_rbuf(sink)).get_segment_buffer();
    auto [first_metadata_offset, new_len] =
        MetadataManager::get_offset_info(l_idx, r_idx - l_idx + 1);
    MetadataManager::encode_segment_metadata(
        (char *)&records[l_idx], intermediate_segment_buffer,
        first_metadata_offset, r_idx - l_idx + 1, metadata);
    auto [raw_offset, raw_len, first_offset] =
        LeafVersionManager::get_offset_info(l_idx, r_idx - l_idx + 1);
    LeafVersionManager::encode_segment_versions(
        intermediate_segment_buffer, encoded_segment_buffer, first_offset,
        hopped_idxes, l_idx, r_idx, first_metadata_offset, new_len);
#else
    // segment [l_idx, r_idx]
    auto [raw_offset, raw_len, first_offset] =
        VerMng::get_offset_info(l_idx, r_idx - l_idx + 1);
    VerMng::encode_segment_versions((char *)&records[l_idx],
                                    encoded_segment_buffer, first_offset,
                                    hopped_idxes, l_idx, r_idx);
#endif
    // write segment and unlock
    if (!need_unlock) {
      dsm->write_sync(encoded_segment_buffer, node_addr + raw_offset, raw_len,
                      sink);
    } else if (r_idx == define::leafSpanSize - 1) {
      memcpy(encoded_segment_buffer + raw_len + lock_offset -
                 define::transLeafSize,
             lock_buffer, sizeof(uint64_t)); // unlock
      dsm->write_sync(encoded_segment_buffer, node_addr + raw_offset,
                      raw_len + define::allocationLockSize, sink);
    } else {
      std::vector<RdmaOpRegion> rs(2);
...
#ifdef METADATA_REPLICATION
    // segment [0, r_idx]
    auto intermediate_segment_buffer_1 =
        (dsm->get_rbuf(sink)).get_segment_buffer();
    auto [first_metadata_offset_1, new_len_1] =
        MetadataManager::get_offset_info(0, r_idx + 1);
    MetadataManager::encode_segment_metadata(
        (char *)&records[0], intermediate_segment_buffer_1,
        first_metadata_offset_1, r_idx + 1, metadata);
    auto [raw_offset_1, raw_len_1, first_offset_1] =
        LeafVersionManager::get_offset_info(0, r_idx + 1);
    LeafVersionManager::encode_segment_versions(
        intermediate_segment_buffer_1, encoded_segment_buffer_1, first_offset_1,
        hopped_idxes, 0, r_idx, first_metadata_offset_1, new_len_1);
    // segment [l_idx, SPAN_SIZE-1]
    auto intermediate_segment_buffer_2 =
        (dsm->get_rbuf(sink)).get_segment_buffer();
    auto [first_metadata_offset_2, new_len_2] =
        MetadataManager::get_offset_info(l_idx, define::leafSpanSize - l_idx);
    MetadataManager::encode_segment_metadata(
        (char *)&records[l_idx], intermediate_segment_buffer_2,
        first_metadata_offset_2, define::leafSpanSize - l_idx, metadata);
    auto [raw_offset_2, raw_len_2, first_offset_2] =
        LeafVersionManager::get_offset_info(l_idx,
                                            define::leafSpanSize - l_idx);
    LeafVersionManager::encode_segment_versions(
        intermediate_segment_buffer_2, encoded_segment_buffer_2, first_offset_2,
        hopped_idxes, l_idx, define::leafSpanSize - 1, first_metadata_offset_2,
        new_len_2);
#else
    // segment [0, r_idx]
    auto [raw_offset_1, raw_len_1, first_offset_1] =
        VerMng::get_offset_info(0, r_idx + 1);
    VerMng::encode_segment_versions((char *)&records[0],
                                    encoded_segment_buffer_1, first_offset_1,
                                    hopped_idxes, 0, r_idx);
    // segment [l_idx, SPAN_SIZE-1]
    auto [raw_offset_2, raw_len_2, first_offset_2] =
        VerMng::get_offset_info(l_idx, define::leafSpanSize - l_idx);
    VerMng::encode_segment_versions(
        (char *)&records[l_idx], encoded_segment_buffer_2, first_offset_2,
        hopped_idxes, l_idx, define::leafSpanSize - 1);
#endif
    ...
    if (!need_unlock) {
      ...
      memcpy(encoded_segment_buffer_2 + raw_len_2 + lock_offset -
                 define::transLeafSize,
             lock_buffer, sizeof(uint64_t)); // unlock
...
void RolexIndex::entry_write_and_unlock(LeafNode *leaf, const int idx,
                                        const GlobalAddress &node_addr,
                                        const GlobalAddress &locked_leaf_addr,
                                        uint64_t *lock_buffer, CoroPull *sink) {
  ...
  auto get_info_and_encode_versions = [=, &entry, &metadata](int idx) {
    auto intermediate_segment_buffer =
        (dsm->get_rbuf(sink)).get_segment_buffer();
    auto [first_metadata_offset, new_len] =
        MetadataManager::get_offset_info(idx);
    MetadataManager::encode_segment_metadata(
        (char *)&entry, intermediate_segment_buffer, first_metadata_offset, 1,
        metadata);
    auto [raw_offset, raw_len, first_offset] =
        LeafVersionManager::get_offset_info(idx);
    LeafVersionManager::encode_segment_versions(
        intermediate_segment_buffer, encoded_entry_buffer, first_offset,
        std::vector<int>{idx}, idx, idx, first_metadata_offset, new_len);
  ...
  VerMng::encode_entry_versions((char *)&entry, encoded_entry_buffer,
                                first_offset);
#endif
  // write entry and unlock
  auto lock_offset = get_lock_info();
  if (idx == define::leafSpanSize - 1 && node_addr == locked_leaf_addr) {
    memcpy(encoded_entry_buffer + raw_len + lock_offset - define::transLeafSize,
           lock_buffer, sizeof(uint64_t)); // unlock
    dsm->write_sync(encoded_entry_buffer, node_addr + raw_offset,
                    raw_len + define::allocationLockSize, sink);
  } else {
    std::vector<RdmaOpRegion> rs(2);
...
#ifdef SPECULATIVE_READ
bool RolexIndex::speculative_read(GlobalAddress &leaf_addr,
                                  std::pair<int, int> range, const Key &k,
                                  Value &v, LeafNode *&leaf,
                                  int &speculative_idx, int &read_leaf_cnt,
                                  CoroPull *sink) {
  auto &syn_leaf_addrs = coro_syn_leaf_addrs[sink ? sink->get() : 0];
  ...
  auto leaf_buffer = (dsm->get_rbuf(sink)).get_leaf_buffer();
  leaf = (LeafNode *)leaf_buffer;
  if (idx_cache->search_idx_from_cache(leaf_addr, range.first, range.second, k,
                                       speculative_idx)) {
    ...
    read_leaf_cnt++;
    if (speculative_idx >= define::leafSpanSize) {
      leaf_entry_read(syn_leaf_addrs[leaf_addr],
                      speculative_idx - define::leafSpanSize, raw_leaf_buffer,
                      leaf_buffer, sink);
    } else {
      leaf_entry_read(leaf_addr, speculative_idx, raw_leaf_buffer, leaf_buffer,
                      sink);
    }
...
void RolexIndex::leaf_entry_read(const GlobalAddress &leaf_addr, const int idx,
                                 char *raw_leaf_buffer, char *leaf_buffer,
                                 CoroPull *sink) {
...
re_read:
  ...
  if (!LeafVersionManager::decode_segment_versions(
          raw_entry_buffer, intermediate_entry_buffer, first_offset, 1,
          first_metadata_offset, new_len, entry_node_version)) {
    goto re_read;
  }
  MetadataManager::decode_segment_metadata(
      intermediate_entry_buffer, (char *)&(leaf->records[idx]),
      first_metadata_offset, 1, leaf->metadata);
  return;
#else
  auto [raw_offset, raw_len, first_offset] = VerMng::get_offset_info(idx);
  auto raw_entry_buffer = raw_leaf_buffer + raw_offset;
re_read:
  ...
  // consistency check; note that: (segment_size_l > 0) => there are two
  // segments
  if (!VerMng::decode_header_versions(raw_leaf_buffer, leaf_buffer,
                                      metadata_node_version) ||
      !VerMng::decode_segment_versions(raw_entry_buffer,
                                       (char *)&(leaf->records[idx]),
                                       first_offset, 1, entry_node_version) ||
...
void RolexIndex::run_coroutine(GenFunc gen_func, WorkFunc work_func,
                               int coro_cnt, Request *req, int req_num) {
  assert(coro_cnt <= MAX_CORO_NUM);
  // define coroutines
  for (int i = 0; i < coro_cnt; ++i) {
    RequstGen *gen = gen_func(dsm, req, req_num, i, coro_cnt);
    workers.emplace_back(
        [=](CoroPull &sink) { coro_worker(sink, gen, work_func); });
  }
...
void RolexIndex::coro_worker(CoroPull &sink, RequstGen *gen,
                             WorkFunc work_func) {
...
size_t RolexIndex::get_syn_cache_size() {
  ...
  size_t total_bytes = total_elements * (payload_size + tree_overhead);
  total_bytes +=
      (MAX_CORO_NUM + 1) * sizeof(std::map<GlobalAddress, GlobalAddress>);
  return total_bytes;
#else
  return 0;
#endif
